...
export default function Hero() {
  ...
  }, [activeTab, currentConfig, selectedProduct]); // <-- Kembalikan argumen lengkapnya di sini agar ukurannya konstan
  ...
  const handleOrderWhatsApp = () => {
    // Validasi dasar agar user mengisi User ID / Nomor terlebih dahulu
    if (!userId.trim()) {
      alert(`Silakan isi ${currentConfig?.inputPlaceholder || "ID / Nomor"} Anda terlebih dahulu!`);
      return;
    }

    const nomorWA = "6281931194133"; // Ganti dengan nomor WhatsApp ElangShop milikmu
    
    // Menyusun teks pesan rapi dengan format cetak tebal (*) sesuai pilihan pembeli
    const pesanTeks = 
    ...
    // Encode string agar aman dikirimkan lewat tautan URL browser
    const linkWhatsApp = `https://wa.me/${nomorWA}?text=${encodeURIComponent(pesanTeks)}`;
    
    // Buka tautan WhatsApp di tab peramban baru
            ...
            <div className="w-full max-w-[540px] bg-[#0E1628]/85 border border-white/10 rounded-[28px] p-6 backdrop-blur-xl shadow-2xl">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      value={userId}
                      onChange={(e) => setUserId(e.target.value)} // Menghubungkan input teks ke state react
                ...
                <div className="flex gap-3 pt-1">
                  <button 
                    onClick={handleOrderWhatsApp} // Menjalankan fungsi pemicu tautan pesan di sini
                    className="flex-1 h-[52px] rounded-2xl bg-[#FACC15] text-black font-black text-[13px] hover:bg-[#EAB308] transition shadow-lg shadow-[#FACC15]/10 active:scale-[0.99]"
                  >
                    Beli Sekarang
                  </button>
